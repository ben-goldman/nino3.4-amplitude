...
letts = np.array((("a) LENS", "b) xGHG"), ("c) xAER", "d) xBMB"), ("e) xLULC", "f) xO3")))
for m in range(3):
    for n in range(2):
...
axx[2, 1].plot(date_1932, bs_sum_mean)
axx[2, 1].plot(date_2172, ff_var_mean-np.nanmean(ff_var_mean))
...
for m in range(3):
    for n in range(2):
        ...
        figlett(axx[m, n], letts[m, n])
        axx[m, n].hlines(0, 1900, 2100, alpha=.5, linewidth=1)
