...
def three_contours(set1, set2, set3, ov1, ov2, ov3, ov4, ov5, ov6, suptitle, xlabel, xscale):
    cmap = plt.get_cmap('coolwarm')
    norm = colors.Normalize(vmin=-1, vmax=1)
    cf = cm.ScalarMappable(norm=norm, cmap=cmap)

    fig, (ax1, ax2, ax3) = plt.subplots(nrows=3, sharex=True, figsize=size2)
    ax1.contourf(xscale, depth[:36], set1[:36], 32,  cmap=cmap, vmin=-1, vmax=1)
    ax1.contourf(xscale, depth[:36], ov1[:36], cmap=bool_white)
    ax1.contourf(xscale, depth[:36], ov4[:36], cmap=bool_gray)
    ax1.set_ylabel("Depth m")
    ax1.invert_yaxis()
    ax1.set_yscale("log")
    figlett(ax1, "a ff")

    ax2.contourf(xscale, depth[:36], set2[:36], 32, cmap=cmap, vmin=-1, vmax=1)
    ax2.contourf(xscale, depth[:36], ov2[:36], cmap=bool_white)
    ax2.contourf(xscale, depth[:36], ov5[:36], cmap=bool_gray)
    ax2.set_ylabel("Depth m")
    ax2.invert_yaxis()
    ax2.set_yscale("log")
    figlett(ax2, "b xghg")

    ax3.contourf(xscale, depth[:36], set3[:36], 32, cmap=cmap, vmin=-1, vmax=1)
    ax3.contourf(xscale, depth[:36], ov3[:36], cmap=bool_white)
    ax3.contourf(xscale, depth[:36], ov6[:36], cmap=bool_gray)
