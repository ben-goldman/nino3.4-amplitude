...
def figlett(ax, a):
    ax.text(0.01, 0.95, a, transform=ax.transAxes, va='top')

# %%
fig, (axx) = plt.subplots(3, 2, sharex='col', sharey="row", figsize=size1)
plt.suptitle("Nino 3.4 20-Year Variance")
fig.subplots_adjust(wspace=.1)

envelope(axx[0, 0], date_2172, ff_var_mean, ff_var_se, "C0")

envelope(axx[1, 0], date_1932, ghg_var_mean, ghg_var_se, "C1")

envelope(axx[2, 0], date_1932, aer_var_mean, aer_var_se, "C2")

envelope(axx[0, 1], date_1320, bmb_var_mean, bmb_var_se, "C3")

envelope(axx[1, 1], date_1320, luc_var_mean, luc_var_se, "C4")
...
fig.suptitle("Nino 3.4 variance in fully-forced ensembles")

ax.legend(["ff ensemble mean and sterr","oic mean and sterr", "ff 1850-2005 run", "oic 1850-1999 run"])
ax.set_ylim(0.9, 2.0)
...
ghg_bs_mean = np.mean(ghg_bs, axis=1)
...
envelope(axx[0, 0], date_2172, ff_bs_mean, ff_bs_se, "C0")

envelope(axx[1, 0], date_1932, ghg_bs_mean, ghg_bs_se, "C1")

envelope(axx[2, 0], date_1932, aer_bs_mean, aer_bs_se, "C2")

envelope(axx[0, 1], date_1320, bmb_bs_mean, bmb_bs_se, "C3")

envelope(axx[1, 1], date_1320, luc_bs_mean, luc_bs_se, "C4")

envelope(axx[2, 1], date_612, ozo_bs_mean, ozo_bs_se, "C5")
...
ax.legend(["Full Forcing", "Greenhouse Gasses", "Aerosols", "Biomass Burning", "Land Use/Cover", "Ozone"])
...
ax.legend(["Full Forcing", "Greenhouse Gasses", "Aerosols"])

fig.tight_layout()
fig.subplots_adjust(top=.94)
fig.savefig("6.pdf", format="pdf")
...
ax.set_xlabel("Date")
ax.set_ylabel("Variance")
fig.suptitle("FF Ensemble Members Variance")
...
fig = plt.figure(figsize=size1)
ax = fig.add_subplot(111)
ax.plot(x, ctl_var_amoc_1_pdf)
ax.plot(x, ctl_var_amoc_1_pdf_)
ax.plot(x, ctl_var_amoc_2_pdf)
ax.plot(x, ctl_var_amoc_2_pdf_)
ax.legend([u"AMOC < \u03C3", u"AMOC > \u03C3", u"AMOC < 2\u03C3", u"AMOC > 2\u03C3"])
fig.suptitle("Control Nino 3.4 20-year variance PDF for various AMOC distributions")
ax.set_ylabel("Frequency")
ax.set_xlabel("Variance")

fig.tight_layout()
fig.subplots_adjust(top=.94)
fig.savefig("16.pdf", format="pdf")
...
fig = plt.figure(figsize=size1)
ax = fig.add_subplot(111)
ax.plot(x, ctl_var_amo_1_pdf)
ax.plot(x, ctl_var_amo_1_pdf_)
ax.plot(x, ctl_var_amo_2_pdf)
ax.plot(x, ctl_var_amo_2_pdf_)
